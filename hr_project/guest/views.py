...
def addemployee(request,id=0):
    if request.method == "POST":
        ...
        if Login.objects.filter(employee_username = emp_login).exists():
            ...
            employeelist.employee_mail = emp_mail
            ...
            with connection.cursor() as cursor:
                sql_query = "SELECT (id) FROM `guest_hrleavestatus` ORDER BY id DESC LIMIT 1"
                cursor.execute(sql_query)
                result = cursor.fetchone()
                leaveid = result[0]
                
                hrleavestatuslist = hrleavestatus.objects.get(id=leaveid)
                sick_leave = hrleavestatuslist.sick_leaves
                casual_leave = hrleavestatuslist.casual_leaves
                halfday = hrleavestatuslist.half_day_leaves
                
            leavestatuslist.emp_id = employee_id
            leavestatuslist.sick_leave = sick_leave
            leavestatuslist.casual_leave = casual_leave
            leavestatuslist.half_day = halfday
            leavestatuslist.paid_leave = "0"
            leavestatuslist.status_upto = next_year
            
            leavestatuslist.save()
            with connection.cursor() as cursor:
                sql_query1 = "SELECT (id) FROM `guest_hrmonthleavestatus` ORDER BY id DESC LIMIT 1"
                cursor.execute(sql_query1)
                result1 = cursor.fetchone()
                leaveid1 = result1[0]
            
            hrmonthlist = hrmonthleavestatus.objects.get(id=leaveid1)
            sick_leave_month = hrmonthlist.sick_leaves
            casual_leave_month = hrmonthlist.casual_leaves
            halfday_month = hrmonthlist.half_day_leaves
                    
            leavestatuslistformonth = LeaveStatusMonth()
            leavestatuslistformonth.emp_id = employee_id
            leavestatuslistformonth.sick_leave = sick_leave_month
            leavestatuslistformonth.casual_leave = casual_leave_month
            leavestatuslistformonth.half_day = halfday_month
        ...
        employeelist.delete()
        loginlist = Login.objects.get(emp_id=id)
        loginlist.delete()
...
def forgetpassword(request):
    if request.method == "POST":
        ...
        print(loginid)
        if Login.objects.filter(employee_username=loginid).exists():
            login_obj = Login.objects.get(employee_username=loginid)
            emp_id = login_obj.emp_id
            employee_obj = Employees.objects.get(employee_id=emp_id)
            email_id = employee_obj.employee_mail
            ...
            otplist = otp()
            otplist.otp = otp_number
            otplist.date = date_now
            otplist.time = time
            otplist.loginid = loginid
            otplist.save()
...
def otp_list(request):
    if request.method == "POST":
        ...
        integer_otp = int(otpnumber)
        id = request.POST.get('ID')
        with connection.cursor() as cursor:
            sql_query = "SELECT (otp) FROM `guest_otp` WHERE loginid=%s ORDER BY otp_id DESC LIMIT 1"
            cursor.execute(sql_query,[id])
            result = cursor.fetchone()
            otp_database = result[0]
            integer_otp_database = int(otp_database)
            
            if integer_otp == integer_otp_database:
                otplist = otp.objects.filter(loginid=id)
                otplist.delete()
                return JsonResponse({"success":True})
            else:
                return JsonResponse({"fail":True})
...
@csrf_exempt            
def clearotp(request,id=0):
    if request.method =="DELETE":
        otplist = otp.objects.filter(loginid=id)
        otplist.delete()
        return JsonResponse({"success":True})
    
def gethrleave(request):
    if request.method == "GET":
        leavelist = hrleavestatus.objects.all()
        data = [
            {
                'id':item.id,
                'casual':item.casual_leaves,
                'sick':item.sick_leaves,
                'half':item.half_day_leaves
            }
            for item in leavelist
        ]
        return JsonResponse(data,safe=False)
...
@csrf_exempt   
def updateleaveall(request,id):
    if request.method == "POST":
        casual = request.POST.get('casual')
        sick = request.POST.get('sick')
        half = request.POST.get('half')
        
        hrlist = hrleavestatus.objects.get(id=id)
        old_casual_leave = hrlist.casual_leaves
        old_sick_leave = hrlist.sick_leaves
        old_half_leave = hrlist.half_day_leaves
        
        casual_leaves_to_be_added = int(casual) - int(old_casual_leave)
        sick_leaves_to_be_added = int(sick) - int(old_sick_leave)
        half_leaves_to_be_added = int(half) - int(old_half_leave)
            
        hrlist.casual_leaves = casual
        hrlist.sick_leaves = sick
        hrlist.half_day_leaves = half
        hrlist.save()
        
        leavestatuslist = LeaveStatus.objects.all()
        for data in leavestatuslist:
            data.sick_leave += sick_leaves_to_be_added
            data.casual_leave += casual_leaves_to_be_added
            data.half_day += half_leaves_to_be_added
            data.save()
            
            if data.sick_leave < 0:
                emp_id = data.emp_id
                excess_sick_leave = int(0) - int(data.sick_leave)
                data.excess_leave = excess_sick_leave
                data.sick_leave = 0
                data.save()
                employeelist = Employees.objects.get(employee_id=emp_id)
                basepackage = employeelist.base_package
                reduction_amount = int(basepackage/30)*int(excess_sick_leave)
                with connection.cursor() as cursor:
                    sql_query = "SELECT MONTH(CURRENT_DATE())"
                    cursor.execute(sql_query)
                    result = cursor.fetchone()
                    month = result[0]
                other = OtherReduction()
                other.emp_id = emp_id
                other.deducted_amount = reduction_amount
                other.deducted_reason = "EXCESS SICK LEAVE"
                other.reduction_for = month
                other.save()
                
            if data.casual_leave < 0:
                emp_id = data.emp_id
                excess_casual_leave = int(0) - int(data.casual_leave)
                data.excess_leave = excess_casual_leave
                data.casual_leave = 0
                data.save()
                
                employeelist = Employees.objects.get(employee_id=emp_id)
                basepackage = employeelist.base_package
                reduction_amount = int(basepackage/30)*int(excess_casual_leave)
                with connection.cursor() as cursor:
                    sql_query1 = "SELECT MONTH(CURRENT_DATE())"
                    cursor.execute(sql_query1)
                    result1 = cursor.fetchone()
                ...
                other.emp_id = emp_id
                other.deducted_amount = reduction_amount
                other.deducted_reason = "EXCESS CASUAL LEAVE"
                other.reduction_for = month1
                other.save()
                
            if data.half_day < 0:
                emp_id = data.emp_id
                excess_half_day = int(0) - int(data.half_day)
                data.excess_leave = excess_half_day
                data.half_day = 0
                data.save()
                employeelist = Employees.objects.get(employee_id=emp_id)
                basepackage = employeelist.base_package
                reduction_amount = int(basepackage/60)*int(excess_half_day)
                with connection.cursor() as cursor:
                    sql_query2 = "SELECT MONTH(CURRENT_DATE())"
                    cursor.execute(sql_query2)
                ...
                other.emp_id = emp_id
                other.deducted_amount = reduction_amount
                other.deducted_reason = "EXCESS HALF DAY LEAVE"
                other.reduction_for = month2
                other.save()
...
@csrf_exempt   
def hrleaveget(request):
    if request.method == "GET":
        if hrleavestatus.objects.all().exists():
            return JsonResponse({"success":True})
        else:
            return JsonResponse({"fail":True})
...
@csrf_exempt    
def hrmonthleave(request):
    if request.method == "GET":
        if hrmonthleavestatus.objects.all().exists():
            hrlist = hrmonthleavestatus.objects.all()
            data = [
                {
                    'id':item.id,
                    'casual':item.casual_leaves,
...
@csrf_exempt
def updatemonthleave(request,id):
    if request.method == "POST":
        casual = request.POST.get('casual')
        sick = request.POST.get('sick')
        half = request.POST.get('half')
        hrlist = hrmonthleavestatus.objects.get(id=id)
        old_casual_leave = hrlist.casual_leaves
        old_sick_leave = hrlist.sick_leaves
        old_half_leave = hrlist.half_day_leaves
        ...
        leavestatuslist = LeaveStatusMonth.objects.all()
        for data in leavestatuslist:
            data.sick_leave += sick_leaves_to_be_added
            data.casual_leave += casual_leaves_to_be_added
            data.half_day += half_leaves_to_be_added
